...
func NewAMQPEventEmitter(conn *amqp.Connection, exchange string) (msgqueue.EventEmitter, error) {
	...
	log.Printf("NewAMQPEventEmitter exchange=%s\n", exchange)
	err := emitter.setup()
	if err != nil {
		return nil, err
	}
...
func (a *amqpEventEmitter) setup() error {
	...
	log.Println("amqpEventEmitter.setup() - channel ExchangeDeclare")
...
func (a *amqpEventEmitter) Emit(event msgqueue.Event) error {
	...
	log.Printf("amqpEventEmitter.Emit() %s - channel Publish to %s\n", event.EventName(), a.exchange)
