...
func ExtractConfiguration(filename string) (ServiceConfig, error) {
	/*var (
		buf    bytes.Buffer
		logger = log.New(&buf, "ExtractConfiguration: ", log.Lshortfile)
	)*/
	...
	log.Printf("Rest=%v, DBtype=%v, DBConnection=%v, MessageBrokerType=%v, AMQPMessageBroker=%v, KafkaMB=%v", conf.RestfulEndpoint, conf.Databasetype, conf.DBConnection, conf.MessageBrokerType, conf.AMQPMessageBroker, conf.KafkaMessageBrokers)
